...
inline void imshow(const unsigned char* ptr, const int rows, const int columns, const int colors, const std::map<std::string, std::string>& keywords = {}, PyObject** out = nullptr) { detail::imshow((void*)ptr, NPY_UINT8, rows, columns, colors, keywords, out); }

inline void imshow(const float* ptr, const int rows, const int columns, const int colors, const std::map<std::string, std::string>& keywords = {}, PyObject** out = nullptr) { detail::imshow((void*)ptr, NPY_FLOAT, rows, columns, colors, keywords, out); }
...
bool scatter_colored(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericColors>& colors,
                     const double s = 1.0, // The marker size in points**2
                     const std::map<std::string, std::string>& keywords = {})
{
    detail::_interpreter::get();
...
template<typename Numeric>
bool errorbar(const std::vector<Numeric>& y, const std::vector<Numeric>& yerr, const std::map<std::string, std::string>& keywords = {})
{
    std::vector<Numeric> x(y.size());
    for (size_t i = 0; i < x.size(); ++i)
        x.at(i) = i;
    return errorbar(x, y, yerr, keywords);
}
...
inline void cla()
{
    ...
    PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_cla, detail::_interpreter::get().s_python_empty_tuple);
...
inline bool plot(const std::vector<double>& x, const std::vector<double>& y, const std::map<std::string, std::string>& keywords) { return plot<double>(x, y, keywords); }
